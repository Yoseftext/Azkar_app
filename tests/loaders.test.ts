...
test('azkar loader normalizes categories and supports text filtering', async () => {
...
test('dua loader normalizes categories and summaries', async () => {
  ...
  const filtered = filterLoadedDuaCategories(categories, categories[0].title);
  assert.ok(filtered.some((category) => category.slug === categories[0].slug));
});

test('names loader supports search and deterministic day selection', async () => {
  const items = await loadAllahNames();
  assert.ok(items.length > 0);
  const match = filterLoadedAllahNames(items, `${items[0].name} ${items[0].order}`);
  assert.ok(match.some((item) => item.id === items[0].id));
